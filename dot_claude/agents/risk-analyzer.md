...
spec.md に「実コード L〜〜では...」「既存処理は〜〜」「`classNames` には〜〜が入る」のような実コード参照記述がある場合、該当ファイルを `Read` で開き、spec.md 記述と実コード挙動が一致するか確認する。乖離があれば直感的リスク把握の前に「spec.md と実コードの乖離」を最優先の懸念として記録し、リスク分析レポートで spec.md 訂正提案として明記する。リスクスコアリングは spec.md の誤記前提で進めると的外れになるため、前提合わせを先行させる。
