...
- 1段検証スキップ（共通契約 C-4 違反: 強表現「最有力候補」「主犯確定」を机上推測で使用）→ **Major**
- 「報告+判断を仰ぐ」パターンの逸脱（共通契約 C-3 違反: スコープ外を勝手に拡張）→ **Major**
...
- 初手スコープの切り方が広すぎ/狭すぎ（YAGNI観点）→ **Minor 〜 Major**
  - Rule Adherence の「スコープ外を勝手に拡張」（拡張の**違反**）とは別軸。ここでは**そもそもの初手宣言スコープの適切さ**を見る
  - 広すぎ例: バグ修正依頼に対し初手で周辺リファクタまで含めた計画を宣言 / 発生し得ないエラーへのハンドリング追加を初手計画に含めた / 「念のため」で仕様外の防御コードを含めた
  - 狭すぎ例: 前提となる別レイヤー修正を初手宣言から漏らし、後からユーザー指摘で追加した
