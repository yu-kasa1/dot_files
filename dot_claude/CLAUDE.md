...
- よほど簡単な改修/修正でない限り、**設計フェーズと実装フェーズでセッションを分ける**こと。
  - 設計セッション: requirement-definer → spec-writer → risk-analyzer → task-writer → code-reviewer（設計レビュー）
  - 実装セッション: coder（←→ impact-analyzer） → test-writer → code-reviewer（コードレビュー） → doc-updater → retrospective
...
### 呼び出しルール
- **仕様作成完了後**：`@spec-writer`の完了後、`@task-writer`の前に呼び出すことを推奨します。
- **設計レビュー時**：`@code-reviewer`の設計レビューと併用可能です。
...
## retrospective
開発完了後にふりかえりを行い、学びを構造化して記録するエージェントです。
### 呼び出しルール
- **パイプライン最終段**：`@doc-updater`の完了後に呼び出すことを推奨します。
- **大きなバグ修正後**：手戻りや方針転換があった場合に呼び出します。
- **対応フロー**：
  1. spec.md、tasks.md、git log、レビュー指摘等を読み込み
  2. Keep（うまくいったこと）/ Problem（うまくいかなかったこと）/ Try（次に試すこと）で整理
  3. 再利用可能なパターン・アンチパターンを抽出
  4. ユーザー確認後、`~/.claude/retrospectives/{プロジェクト名}/` に記録
