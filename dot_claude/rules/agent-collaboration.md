...
両方の完了後 → task-writer
```
- 両エージェントとも spec.md を入力とし、互いに依存しない
- 両方の結果をユーザーが確認した後、必要な修正を spec.md に反映してから task-writer へ進む

### 実装フェーズ: coder全タスク完了後
```
coder 全タスク完了
  ├── code-reviewer（コードレビュー）  ← コード + spec.md を読んで品質チェック
  ├── security-reviewer（セキュリティレビュー） ← コード を読んでセキュリティチェック
  └── test-writer（テストケース作成）  ← spec.md を読んでテスト設計
全エージェント完了後 → 修正対応 → doc-updater
```
- 3エージェントとも「完成したコード + spec.md」を入力とし、互いに依存しない
...
### 呼び出しルール
- **実装着手前**：`@coder`が各タスク着手前に、変更対象の影響範囲を確認したいときに呼び出します。
...
### 呼び出しルール
- **実装完了時**：`@code-reviewer`・`@test-writer`と並列でセキュリティレビューを実施します。
...
- **対応フロー**：
  ...
  2. 各タスクを順次実装
  3. **タスクごとにユーザー承認を待つ**
  4. 全タスク完了後、`@code-reviewer` + `@security-reviewer` + `@test-writer` を並列で呼び出し
